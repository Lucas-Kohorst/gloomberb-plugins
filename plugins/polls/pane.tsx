...
import { useShortcut, useAutoRefresh } from "gloomberb/react";
...
function PollTrend({
  ...
  const dateWidth = 8;
  const valWidth = 5;
  const barWidth = Math.max(8, lineWidth - dateWidth - valWidth - 12);
  const maxPct = Math.max(...trendData.points.map((p) => p.value), 1);

  // Build a lookup so we can show the MA alongside each raw point.
  const maByDate = new Map(trendData.ma.map((m) => [m.date, m.value]));
    ...
    <Box flexDirection="column" height={height}>
      ...
      <ScrollBox flexGrow={1} scrollY>
        <Box flexDirection="column" paddingX={1} gap={0}>
          {trendData.points.map((point, i) => {
            const maValue = maByDate.get(point.date);
            return (
              <Box key={`${point.date}-${i}`} flexDirection="row" height={1} gap={1}>
                <Box width={dateWidth}>
                  <Text fg={colors.textDim}>{formatPollDate(point.date)}</Text>
                </Box>
                <Box width={valWidth} justifyContent="flex-end" flexDirection="row">
                  <Text fg={colors.text}>{point.value.toFixed(1)}</Text>
                </Box>
                <Box width={barWidth}>
                  <AnswerBar
                    pct={point.value}
                ...
                {maValue != null ? (
                  <Text fg={colors.positive}>avg {maValue.toFixed(1)}</Text>
                ) : null}
              </Box>
            );
          })}
