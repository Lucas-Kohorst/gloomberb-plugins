...
import { type InputRenderable, type ScrollBoxRenderable } from "gloomberb/ui";
import {
  useAppDispatch,
  useAppSelector,
  useInputCapture,
...
export function usePredictionMarketsController({
  ...
  const [hideHeader] = usePaneSettingValue<boolean>("hideHeader", false);
  const [defaultBrowseTab] = usePaneSettingValue<PredictionBrowseTab>(
    "defaultBrowseTab",
    "top",
  );
  const paneSettings = useMemo(
    ...
    [columnIds, defaultBrowseTab, hideHeader, hideTabs, lockedVenueScope],
  );
  ...
  useInputCapture(focused && searchFocused);
  ...
  const poll = { intervalMs: 5 * 60_000 };
