...
function sortBookRows(
  rows: BookRow[],
  preference: SortPreference<BookColumnId>,
): BookRow[] {
  if (!preference.columnId) return [...rows];
  return rows
    .map((row, index) => ({ row, index }))
    .sort((left, right) => {
      const value = (row: BookRow) => {
...
function nextBookSort(
  current: SortPreference<BookColumnId>,
  columnId: BookColumnId,
): SortPreference<BookColumnId> {
  const initial = columnId === "outcome" || columnId === "side" ? "asc" : "desc";
  if (current.columnId !== columnId) return { columnId, direction: initial };
  if (current.direction === initial) {
    return { columnId, direction: initial === "asc" ? "desc" : "asc" };
  }
  return { columnId: null, direction: initial };
}
...
export function PredictionMarketBookView({
  ...
  const rows = useMemo(
    () => sortBookRows(naturalRows, sortPreference),
  ...
  return (
    <DataTableView<BookRow, BookColumn>
      ...
      onHeaderClick={(columnId) => setSortPreference((current) => nextBookSort(
        current,
